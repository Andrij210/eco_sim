...
class Species:
    def __init__(self, name, energy_needs=1, reproduction_rate=0.1):
        self.name = name
...
# --- Підкласи Species ---
class Plant(Species):
    def __init__(self, name, energy_production=2, reproduction_rate=0.05):
        super().__init__(name, energy_needs=0, reproduction_rate=reproduction_rate)
...
class Animal(Species):
    def __init__(self, name, energy_needs=2, prey=None, reproduction_rate=0.08):
        super().__init__(name, energy_needs, reproduction_rate)
...
class Microorganism(Species):
    def __init__(self, name, decomposition_rate=0.2, reproduction_rate=0.15):
        super().__init__(name, energy_needs=0.5, reproduction_rate=reproduction_rate)
...
# --- Клас Environment ---
class Environment:
    def __init__(self, name, climate="помірний", resources=100):
        self.name = name
    ...
    def simulate_interactions(self):
        ...
        for species in self.species:
            ...
            if isinstance(species, Plant):
                ...
                print(f"{species.name} спожило {energy_consumed:.2f} енергії, полювало та отримало {energy_gained:.2f} енергії.")
            elif isinstance(species, Microorganism):
                energy_consumed = species.consume_energy()
                resources_gained = species.decompose(self)
                self.resources += resources_gained
                self.resources -= energy_consumed
                print(f"{species.name} спожило {energy_consumed:.2f} енергії та розклало, додавши {resources_gained:.2f} ресурсів.")
            print(species)
    ...
    for i in range(5):
        ...
        for env_name, env in environments.items():
            ...
            for species in env.species:
                species_data = {"type": species.__class__.__name__, "name": species.name, "population": species.population}
                if isinstance(species, Plant):
